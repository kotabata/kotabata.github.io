...
// CSVファイルのパス
const publishedPapersPath = path.join(
  __dirname,
...
const researchProjectsPath = path.join(
  __dirname,
  "../data/rm_research_projects.csv",
);
...
const visiblePresentations = presentationsResult.data.filter((presentation) => {
  if (
    presentation.hasOwnProperty("公開の有無") &&
    presentation["公開の有無"] === "closed"
  ) {
    return false;
  }
  return true;
...
const formattedPresentations = visiblePresentations.map((presentation) => {
  ...
  return {
    id:
      presentation.ID ||
      `presentation-${Math.random().toString(36).substr(2, 9)}`,
    title:
      presentation["タイトル(英語)"] ||
      presentation["タイトル(日本語)"] ||
      "Untitled",
    titleJa:
      presentation["タイトル(日本語)"] ||
      presentation["タイトル(英語)"] ||
    ...
    conference:
      presentation["会議名(英語)"] || presentation["会議名(日本語)"] || "",
    conferenceJa:
      presentation["会議名(日本語)"] || presentation["会議名(英語)"] || "",
...
const visibleResearchProjects = researchProjectsResult.data.filter(
  (researchProject) => {
    if (
      researchProject.hasOwnProperty("公開の有無") &&
      researchProject["公開の有無"] === "closed"
    ) {
      return false;
    }
    return true;
...
const formattedResearchProjects = visibleResearchProjects.map(
  (researchProject) => {
    return {
      id:
        researchProject.ID ||
        `researchProject-${Math.random().toString(36).substr(2, 9)}`,
      title:
        researchProject["制度名(英語)"] ||
        researchProject["制度名(日本語)"] ||
        "",
      titleJa:
        researchProject["制度名(日本語)"] ||
        researchProject["制度名(英語)"] ||
        "",
      subject:
        researchProject["研究種目(英語)"] ||
        researchProject["研究種目(日本語)"] ||
        "",
      subjectJa:
        researchProject["研究種目(日本語)"] ||
        researchProject["研究種目(英語)"] ||
        "",
      funder:
        researchProject["提供機関(英語)"] ||
        researchProject["提供機関(日本語)"] ||
        "",
      funderJa:
        researchProject["提供機関(日本語)"] ||
        researchProject["提供機関(英語)"] ||
        "",
      description:
        researchProject["タイトル(英語)"] ||
        researchProject["タイトル(日本語)"] ||
        "",
      descriptionJa:
        researchProject["タイトル(日本語)"] ||
        researchProject["タイトル(英語)"] ||
      ...
      yearFrom: researchProject["研究期間(From)"]
        ? researchProject["研究期間(From)"].substring(0, 4)
        : "",
      monthFrom: researchProject["研究期間(From)"]
        ? researchProject["研究期間(From)"].substring(5, 7)
        : "",
      yearTo: researchProject["研究期間(To)"]
        ? researchProject["研究期間(To)"].substring(0, 4)
        : "",
      monthTo: researchProject["研究期間(To)"]
        ? researchProject["研究期間(To)"].substring(5, 7)
        : "",
...
const sortedResearchProjects = formattedResearchProjects.sort(
  (a, b) =>
    yearMonthKey(b.yearFrom, b.monthFrom) -
    yearMonthKey(a.yearFrom, a.monthFrom),
);
...
sortedPresentations.forEach(
  (presentation) => presentation.year && yearsSet.add(presentation.year),
);
...
years.forEach((year) => {
  ...
  const yearPresentations = sortedPresentations.filter(
